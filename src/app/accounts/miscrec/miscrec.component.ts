...
@Component({
  ...
  templateUrl: './miscrec.component.html',
  styleUrls: ['../../shared/myc.style.css','./miscrec.component.css'],
...
this.Getdepartment();
  ...
  insertstudentdtls()
  {
    ...
    console.log(JSON.stringify(this.selected1));

    //console.log('testdata', this.dataSource.data);
    this.recpostdata.rech = this.model;
    this.recpostdata.dbnm = this.tempdbnm;
    this.recpostdata.recd=JSON.stringify(this.courseArray);
    this.recpostdata.payinfo=this.coldetails;
    let clcd=localStorage.getItem('clcd');
    let aycd=localStorage.getItem('aycd');
    this.recpostdata.clcd=clcd;
    this.recpostdata.aycd=aycd;
    
    //console.log("dbnm--dd",this.tabledata);

   // console.log('attendance-data', this.tabledata);
    this.acservice.savemiscreceipt(this.recpostdata)
